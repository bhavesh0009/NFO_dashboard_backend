#!/usr/bin/env python3
"""
Historical data batch processing module.

This module provides functionality for batch processing historical data.
It can be run directly as a script or imported and used by other modules.

Usage:
    # Import and use in other code:
    from scripts.fetch_all_historical_data import fetch_all_historical_data
    fetch_all_historical_data(batch_size=10, limit=50, verbose=True)
    
    # Run as a script:
    python scripts/fetch_all_historical_data.py --batch-size 10 --limit 50 --verbose
"""
...
def setup_logging():
    ...
    logfile(os.path.join(log_dir, f'historical_data_{today}.log'))

def fetch_all_historical_data(batch_size=5, limit=None, verbose=False, interval="ONE_DAY"):
    """
    Fetch and store historical data for all equity tokens.
    ...
    try:
        ...
        manager = HistoricalDataManager(db_manager=db_manager)
        ...
        for i in range(0, total_tokens, batch_size):
            ...
            for _, row in batch.iterrows():
                ...
                historical_data = manager.fetch_historical_data(token, exchange, name, interval=interval)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Fetch and store historical data for all equity tokens")
    ...
    results = fetch_all_historical_data(args.batch_size, args.limit, args.verbose, args.interval)
