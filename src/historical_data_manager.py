"""
Historical data manager for Angel One API.
...
class HistoricalDataManager:
    """Manages fetching and processing of historical equity data."""
    ...
    def _get_date_params(self) -> tuple:
        """
        Get fromdate and todate parameters for historical data query.
        ...
        logger.info(f"Date range for historical data: {from_date} to {to_date}")
        return from_date, to_date
    
    def fetch_historical_data(self, token: str, exchange: str, name: str, interval: str = "ONE_DAY") -> Optional[Dict[str, Any]]:
        """
        Fetch historical data for a single equity token.
        ...
        Returns:
            Optional[Dict[str, Any]]: Historical data response or None if failed
        """
        ...
        try:
            logger.info(f"Fetching historical data for {name} ({token}) with {interval} interval")
            historical_data = self.connector.api.getCandleData(params)
            
            if historical_data.get('status'):
                logger.info(f"✅ Successfully fetched {len(historical_data.get('data', []))} records for {name}")
                return historical_data
            else:
                logger.error(f"❌ Failed to fetch historical data for {name}: {historical_data.get('message', 'Unknown error')}")
                return None
                
        except Exception as e:
            logger.error(f"❌ Error fetching historical data for {name}: {str(e)}")
            return None
    
    def process_historical_data(self, limit: int = 5, interval: str = "ONE_DAY") -> Dict[str, Any]:
        """
        Process historical data for equity tokens.
        ...
        for _, row in equity_tokens.iterrows():
            ...
            # Fetch historical data for this token
            historical_data = self.fetch_historical_data(token, exchange, name, interval=interval)
        ...
        logger.info(f"✅ Historical data processing complete. Success: {results['success']}, Errors: {results['errors']}")
        return results
        
    def fetch_and_store_historical_data(self, limit: int = 5, interval: str = "ONE_DAY") -> Dict[str, Any]:
        """
        Fetch and store historical data for equity tokens.
        ...
        for _, row in equity_tokens.iterrows():
            ...
            if historical_data and historical_data.get('status'):
                ...
                if self.db_manager.store_historical_data(token, name, data):
                    ...
                    results["errors"] += 1
                    logger.error(f"Failed to store historical data for {name} ({token})")
            else:
                results["errors"] += 1
        ...
        logger.info(f"✅ Historical data fetch and store complete. Success: {results['success']}, Errors: {results['errors']}")
        return results
...
if __name__ == "__main__":
    ...
    try:
        ...
        results = manager.process_historical_data(limit=5)
